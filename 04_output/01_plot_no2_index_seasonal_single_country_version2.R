...
# Helper functions -------------------------------------------------------------
    ...
    parse_no2_date <- function(x) {
        parsed_date <- trimws(as.character(x))
        parsed_date <- sub("T.*$", "", parsed_date)
        parsed_date <- sub(" .*$", "", parsed_date)

        output_date <- as.Date(parsed_date, format = "%Y-%m-%d")
        needs_mdy_parse <- is.na(output_date)

        if (any(needs_mdy_parse)) {
            output_date[needs_mdy_parse] <- as.Date(
                parsed_date[needs_mdy_parse],
                format = "%m/%d/%Y"
            )
        }
...
# Prepare plotting data --------------------------------------------------------
    ...
    daily_country <- utils::read.csv(
        country_input_path,
        stringsAsFactors = FALSE,
        check.names = FALSE
    ) %>%
        mutate(
            country_iso3 = trimws(as.character(country_iso3)),
            country_name = trimws(as.character(country_name)),
            date = parse_no2_date(date),
            year = format(date, "%Y"),
    ...
    if (nrow(daily_country) == 0) {
        stop(
            "No valid rows remained after parsing the `date` column. ",
            "Check whether the CSV uses `YYYY-MM-DD` or `M/D/YYYY` dates."
        )
    }
