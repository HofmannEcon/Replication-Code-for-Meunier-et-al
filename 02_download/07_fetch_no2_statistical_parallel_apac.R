...
# Settings ---------------------------------------------------------------------
    # General settings ----------------------------------------------------------
        ...
        progress_batch_size <- NA_integer_
...
# Main process -----------------------------------------------------------------
    ...
    resolved_progress_batch_size <- progress_batch_size

    if (is.na(resolved_progress_batch_size) || resolved_progress_batch_size < 1L) {
        resolved_progress_batch_size <- max(parallel_workers * 5L, parallel_workers)
    }
    ...
    if (isTRUE(show_progress_bar)) {
        progress_bar <- utils::txtProgressBar(min = 0, max = total_requests, style = 3)
        on.exit(close(progress_bar), add = TRUE)
    }

    worker_results <- list()
    completed_jobs <- 0L
    cumulative_successful_requests <- 0L
    cumulative_skipped_requests <- 0L
    cumulative_failed_requests <- 0L

    update_progress_tracking <- function(result_batch) {
        log_batch <- bind_rows(empty_download_logs, bind_rows(result_batch)) %>%
        ...
        worker_results <<- c(worker_results, result_batch)
        completed_jobs <<- completed_jobs + length(result_batch)
        cumulative_successful_requests <<- cumulative_successful_requests + sum(
            log_batch$status %in% c("success", "success_empty")
        )
        cumulative_skipped_requests <<- cumulative_skipped_requests + sum(
            log_batch$status == "skipped_existing"
        )
        cumulative_failed_requests <<- cumulative_failed_requests + sum(
            log_batch$status == "failed"
        )

        if (!is.null(progress_bar)) {
            utils::setTxtProgressBar(progress_bar, completed_jobs)
        }

        if (isTRUE(show_progress_summary)) {
            cat(
                sprintf(
                    "\nCompleted: %d/%d | Success: %d | Skipped: %d | Failed: %d\n",
                    completed_jobs,
                    total_requests,
                    cumulative_successful_requests,
                    cumulative_skipped_requests,
    ...
    if (parallel_workers == 1L) {
        for (job_index in seq_len(nrow(job_table))) {
            update_progress_tracking(list(process_job(job_index)))
        }
    } else {
        ...
        all_job_indices <- seq_len(nrow(job_table))
        job_batches <- split(
            all_job_indices,
            ceiling(seq_along(all_job_indices) / resolved_progress_batch_size)
        )

        for (batch_indices in job_batches) {
            result_batch <- parallel::parLapplyLB(
                cluster_object,
                batch_indices,
                process_job
            )

            update_progress_tracking(result_batch)
        }
    }
