...
# Settings ---------------------------------------------------------------------
    # General settings ----------------------------------------------------------
        ...
        full_run_start_date <- as.Date(get_script_arg("start-date", "2019-01-01"))
        full_run_end_date <- as.Date(get_script_arg("end-date", "2019-12-31"))
        requested_parallel_workers <- as.integer(get_script_arg("workers", "4"))
        request_delay_seconds <- as.numeric(get_script_arg("delay-seconds", "0.5"))
        run_label <- paste0(
